...
exports.fundEstimateRouter = void 0;
const express_1 = require("express");
const cache_1 = require("../middleware/cache");
const jsonp_1 = require("../parsers/jsonp");
const router = (0, express_1.Router)();
exports.fundEstimateRouter = router;
...
function isChinaAfterMarketClose() {
    const now = new Date();
...
async function fetchConfirmedNav(code) {
    try {
        return await (0, cache_1.getCachedOrFetch)(cache_1.confirmedNavCache, `confirmed-${code}`, async () => {
            ...
            const rowMatch = html.match(/<tr[^>]*>\s*<td>(\d{4}-\d{2}-\d{2})<\/td>\s*<td[^>]*>([\d.]+)<\/td>\s*<td[^>]*>[\d.]+<\/td>\s*<td[^>]*>([^<]*)<\/td>/);
            if (!rowMatch)
                return null;
            const [, date, navStr, changeStr] = rowMatch;
            const today = getTodayDateString();
            if (date !== today)
                return null; // Not yet published for today
...
function buildEstimateData(json) {
    return {
        code: json.fundcode,
        name: json.name,
...
async function enrichWithConfirmedNav(data) {
    if (!isChinaAfterMarketClose()) {
        return { ...data, navSource: 'estimate' };
    }
...
router.get('/:code', async (req, res, next) => {
    try {
        ...
        const data = await (0, cache_1.getCachedOrFetch)(cache_1.estimateCache, `estimate-${code}`, async () => {
            ...
            const json = (0, jsonp_1.parseJsonp)(text);
...
//# sourceMappingURL=fund-estimate.js.map